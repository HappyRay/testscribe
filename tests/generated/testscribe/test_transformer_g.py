...
import pytest
from testscribe.transformer import can_use_repr, create_callable_model, transform_class, transform_value
...
def test_transform_value_recursive_transform():
    ...
    assert len(result) == 2
    assert result[0] == 1
    assert isinstance(result[1], testscribe.model_type.CallableModel)
...
def test_transform_value_input_value():
    result = transform_value(v=testscribe.value_util.InputValue("a + 1", 2))
