...
from testscribe.code_gen_util import add_indentation, add_indentation_to_str_list, collect_return_values, get_module_name, is_same_value, join_lines, pretty_format_repr_string, remove_empty, spec_contain_param_name_info, translate_special_mock_return_value
...
def test_add_indentation_to_str_list_indent():
...
def test_collect_return_values():
    ...
    m_mock_call_model_1.return_value = testscribe.model_type.ExpressionModel("ignore")
...
def test_translate_special_mock_return_value_ignore():
    result = translate_special_mock_return_value(value=testscribe.model_type.ExpressionModel("ignore"))
    assert isinstance(result, testscribe.ignore.IgnoreReturnValue)
    assert repr(result) == "'Ignored'"


def test_translate_special_mock_return_value_user_triggered_exception():
    result = translate_special_mock_return_value(value=testscribe.model_type.ExpressionModel("throw(Exception())"))
    assert isinstance(result, testscribe.user_triggered_exception.UserTriggeredException)
    assert repr(result) == 'Exception()'


def test_translate_special_mock_return_value_expression():
    result = translate_special_mock_return_value(value=testscribe.model_type.ExpressionModel("1 + 2"))
