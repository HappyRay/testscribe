...
def test_generate_patch_statement_dict_as_replacement():
    input_mock: testscribe.model_type.PatchModel = create_autospec(spec=testscribe.model_type.PatchModel)
    input_mock.target = 't'
...
def test_generate_patch_statement_mock_as_replacement():
    input_mock: testscribe.model_type.PatchModel = create_autospec(spec=testscribe.model_type.PatchModel)
    input_mock.target = 't'
    input_mock.replacement = testscribe.model_type.MockNameModel("m")
    result = generate_patch_str(patches=[input_mock])
    assert result == """\
...
def test_generate_patch_statement_multiple_patch():
    input_mock: testscribe.model_type.PatchModel = create_autospec(spec=testscribe.model_type.PatchModel)
    input_mock_1: testscribe.model_type.PatchModel = create_autospec(spec=testscribe.model_type.PatchModel)
    input_mock.target = 't'
