...
import pytest
from test_scribe.mock_call import get_arg_str_display, get_args_list_internal, get_call_subject, get_default_mock_call_return_value, get_default_return_value_from_models, get_default_return_value_internal, get_func_args_list, get_mock_call_return_type, get_mock_call_signature, get_true_return_value, save_args
...
def test_get_arg_str_display_has_argument():
    ...
    mock_call.args = test_scribe.namedvalues.NamedValues([("p", 1)])
...
def test_get_arg_str_display_empty_argument():
    mock_call: test_scribe.mock_call.MockCall = create_autospec(spec=test_scribe.mock_call.MockCall)
    mock_call.args = test_scribe.namedvalues.NamedValues()
    result = get_arg_str_display(mock_call=mock_call)
    assert result == ''
    mock_call.assert_not_called()
...
def test_get_true_return_value_exception_expression():
    with pytest.raises(Exception) as exception_info:
        get_true_return_value(value=test_scribe.value_util.InputValue(expression="throw Exception()", value=test_scribe.user_triggered_exception.UserTriggeredException(Exception())))
    assert '' == str(exception_info.value)


def test_get_true_return_value_non_exception_expression():
    result = get_true_return_value(value=test_scribe.value_util.InputValue("a + 1", 2))
    assert result == 2
