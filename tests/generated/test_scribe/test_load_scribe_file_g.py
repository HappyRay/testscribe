...
def test_load_mock_call_model_default():
    ...
    assert isinstance(result, test_scribe.model_type.MockCallModel)
...
def test_load_mock_calls_has_call():
    ...
    assert len(result) == 2
    assert isinstance(result[0], test_scribe.model_type.MockCallModel)
    assert repr(result[0]) == "MockCallModel(name='n', parameters=NamedValues([('a', 1)]), return_value=3)"
    assert isinstance(result[1], test_scribe.model_type.MockCallModel)
...
def test_load_mock_model():
    ...
    assert isinstance(result, test_scribe.model_type.MockModel)
...
def test_load_mocks():
    ...
    assert isinstance(result, list)
    assert len(result) == 2
    assert isinstance(result[0], test_scribe.model_type.MockModel)
    assert repr(result[0]) == "MockModel(name='m', spec_str='s', calls=[], attributes={})"
    assert isinstance(result[1], test_scribe.model_type.MockModel)
...
def test_load_one_patch():
    ...
    assert isinstance(result, test_scribe.model_type.PatchModel)
...
def test_load_test():
    ...
    assert isinstance(result, test_scribe.model_type.TestModel)
