...
import test_scribe.gen_mock_verification_code
...
def test_generate_mock_call_list_no_call():
    ...
    assert isinstance(result, test_scribe.gen_mock_verification_code.MockCallList)
...
def test_generate_mock_call_list_two_method_calls_one_with_complex_param():
    ...
    m_mock_call_model.parameters = test_scribe.namedvalues.NamedValues([("p", 1)])
    m_mock_call_model.name = 'm1'
    m_mock_call_model_1.parameters = test_scribe.namedvalues.NamedValues([("p2", test_scribe.model_type.ObjectModel("t", "r", {}))])
...
def test_generate_mock_call_list_verfication():
    result = generate_mock_call_list_verfication(mock_name='m', spec_str='test_scribe.simple.C', mock_calls_name='v', call_list_str='call list str')
    assert result == """\
    v = get_normalized_mock_calls(m, test_scribe.simple.C)
    assert v == [
        call list str
    ]"""
...
def test_generate_mock_call_verification():
    ...
    m_1.name = 'm'
    m_mock_call_model.parameters = test_scribe.namedvalues.NamedValues([('p', test_data.value.object_model_c)])
...
def test_generate_mock_call_verification_statements():
    ...
    m_mock_call_model.parameters = test_scribe.namedvalues.NamedValues()
    m_mock_call_model.name = 'c'
...
def test_generate_one_mock_call_method_call_with_simple_named_param():
    call: test_scribe.model_type.MockCallModel = create_autospec(spec=test_scribe.model_type.MockCallModel)
    call.parameters = test_scribe.namedvalues.NamedValues([("p", 2)])
    call.name = 'm'
...
def test_get_mock_call_params():
    ...
    assert isinstance(result, test_scribe.gen_mock_verification_code.MockCallParams)
...
def test_get_mock_calls_variable_value_has_param_info():
    result = get_mock_calls_variable_value(mock_name='m', spec_str='test_scribe.simple.C')
    assert result == 'get_normalized_mock_calls(m, test_scribe.simple.C)'
...
def test_get_one_complex_param_no_complex_param():
    call_params: test_scribe.gen_mock_verification_code.MockCallParams = create_autospec(spec=test_scribe.gen_mock_verification_code.MockCallParams)
...
def test_get_one_complex_param_named_complex_param():
    ...
    call_params.index_to_value = {}
    result = get_one_complex_param(call_params=call_params, index=1)
    assert isinstance(result, test_scribe.gen_mock_verification_code.ComplexMockCallParam)
...
def test_get_one_mock_call_param_complex_param():
    ...
    assert isinstance(result[2][0], test_scribe.model_type.ObjectModel)
