...
def test_eval_with_injected_names():
    with patch('test_scribe.eval_expression.global_var.g_name_mock_dict', {'a': 2}):
        result = eval_with_injected_names(expression='(a, m, test_data.simple.INT_VALUE)')
    assert result == (2, test_scribe.api.mock_api.m, 1)
...
def test_expand_string_aliases():
    with patch('test_scribe.eval_expression.g_aliases', {'a': 'hello', 'b': 'World'}):
...
def test_process_mock_marker_type_mismatch_throw_error():
    with pytest.raises(test_scribe.error.InputError) as exception_info:
        process_mock_marker(t=typing.Tuple[int, int], v=(test_scribe.api.mock_api.m, 1))
