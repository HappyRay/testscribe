...
import test_scribe.transformer
from test_scribe.result_assertion import generate_assertion, generate_complex_type_assertion, generate_module_assertion, generate_result_assertion, generate_set_assertion, generate_special_collection_assertion


def test_generate_assertion_module_type():
    result = generate_assertion(target='a', value=test_scribe.model_type.ModuleModel("m"))
    assert result == """\
...
def test_generate_assertion_class_result():
    result = generate_assertion(target='a', value=test_scribe.transformer.transform_value(test_data.simple.C(1)))
    assert result == """\
...
def test_generate_complex_type_assertion_module_type():
    result = generate_complex_type_assertion(target='t', value=test_scribe.model_type.ModuleModel("m"))
    assert result == """\
    from types import ModuleType
...
def test_generate_module_assertion():
    result = generate_module_assertion(target='t', value=test_scribe.model_type.ModuleModel("m"))
    assert result == """\
    from types import ModuleType
    assert type(t) == ModuleType
    t.__name__ == 'm'"""
...
def test_generate_result_assertion_with_class_with_class_member():
    result = generate_result_assertion(result=test_scribe.transformer.transform_value(test_data.person.Family(husband=test_data.person.Person("a", 1), wife=test_data.person.Person("b", 2), kids=[])))
    assert result == """\
...
def test_generate_result_assertion_mock():
    result = generate_result_assertion(result=test_scribe.model_type.MockNameModel("a"))
    assert result == """\
...
def test_generate_set_assertion_target_str_not_a_valid_identifier_name():
    result = generate_set_assertion(target='result[1]', value=test_scribe.model_type.SetModel([1, 2]))
    assert result == """\
...
def test_generate_set_assertion():
    result = generate_set_assertion(target='r', value=test_scribe.model_type.SetModel([1, 2]))
    assert result == """\
