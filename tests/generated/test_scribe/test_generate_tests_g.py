...
def test_gen_create_invocation_str():
    ...
    test.init_parameters = test_scribe.namedvalues.NamedValues()
    test.parameters = test_scribe.namedvalues.NamedValues()
    test.exception = None
...
def test_generate_create_instance_statement_no_class():
    result = generate_create_instance_statement(target_class_name='', init_parameters=test_scribe.namedvalues.NamedValues())
    assert result == ''


def test_generate_create_instance_statement():
    result = generate_create_instance_statement(target_class_name='Foo', init_parameters=test_scribe.namedvalues.NamedValues([("a", 1), ("b", 2)]))
...
def test_generate_invocation_statement_method():
    test: test_scribe.model_type.TestModel = create_autospec(spec=test_scribe.model_type.TestModel)
    test.parameters = test_scribe.namedvalues.NamedValues([("a", 1)])
    test.target_class_name = 'C'
    test.exception = None
    test.target_func_name = 'f'
    result = generate_invocation_statement(test=test)
...
def test_generate_mocks_single_mock_without_attributes_and_calls():
    input_mock: test_scribe.model.MockModel = create_autospec(spec=test_scribe.model.MockModel)
...
def test_generate_unit_test_file_no_test():
    ...
    all_tests.tests = []
    with patch('test_scribe.generate_tests.remove_file_if_no_test', m_remove_file_if_no_test):
...
def test_wrap_exception_assertion():
    exception_model: test_scribe.model_type.ExceptionModel = create_autospec(spec=test_scribe.model_type.ExceptionModel)
