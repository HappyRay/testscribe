...
from test_scribe.model_encoder import encode_a_mock, encode_mock_calls, encode_mocks, encode_model, encode_name_and_description, encode_named_values, encode_one_mock_call, encode_one_mock_call_name, encode_one_test, encode_parameters, encode_patches, encode_result, encode_target, encode_tests
...
def test_encode_mock_calls():
    ...
    m_mock_call_model_1.parameters = test_scribe.namedvalues.NamedValues()
...
def test_encode_name_and_description_no_description():
    test_model: test_scribe.model_type.TestModel = create_autospec(spec=test_scribe.model_type.TestModel)
    test_model.name = 'n'
...
def test_encode_named_values():
    result = encode_named_values(values=test_scribe.namedvalues.NamedValues([("", 1), ("b", 2)]))
    assert result == [{'value': 1}, {'name': 'b', 'value': 2}]


def test_encode_named_values_empty_value():
    result = encode_named_values(values=test_scribe.namedvalues.NamedValues())
    assert result == []


def test_encode_one_mock_call_false_return_value_should_persist():
    result = encode_one_mock_call(mock_call=test_scribe.model_type.MockCallModel(name="", parameters=test_scribe.namedvalues.NamedValues(), return_value=False))
    assert result == {'parameters': [], 'return': False}


def test_encode_one_mock_call_no_name_none_return():
    result = encode_one_mock_call(mock_call=test_scribe.model_type.MockCallModel(name="", parameters=test_scribe.namedvalues.NamedValues(), return_value=None))
    assert result == {'parameters': []}


def test_encode_one_mock_call():
    result = encode_one_mock_call(mock_call=test_scribe.model_type.MockCallModel(name="foo", parameters=test_scribe.namedvalues.NamedValues([("a", 1), ("b", 2)]), return_value=3))
...
def test_encode_one_test():
    ...
    test_model.init_parameters = test_scribe.namedvalues.NamedValues()
    test_model.parameters = test_scribe.namedvalues.NamedValues()
...
def test_encode_parameters_has_init_param():
    test_model: test_scribe.model_type.TestModel = create_autospec(spec=test_scribe.model_type.TestModel)
    test_model.init_parameters = test_scribe.namedvalues.NamedValues([("p", 1)])
    test_model.parameters = test_scribe.namedvalues.NamedValues()
    result = encode_parameters(test={'foo': 1}, test_model=test_model)
...
def test_encode_result_exception():
    test_model: test_scribe.model_type.TestModel = create_autospec(spec=test_scribe.model_type.TestModel)
    test_model.exception = test_scribe.model_type.ExceptionModel("a.b.C", "msg")
    result = encode_result(test={}, test_model=test_model)
