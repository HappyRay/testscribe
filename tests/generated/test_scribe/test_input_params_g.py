...
import test_scribe.input_params
import test_scribe.module
from test_scribe.api.mock_api import get_normalized_mock_calls
from unittest.mock import ANY, call, create_autospec
from unittest.mock import patch
from test_scribe.input_params import create_input_params, create_output_dir_for_module, get_scribe_file_path, get_target_class_name, get_target_function_name


def test_create_input_params():
    result = create_input_params(module=test_scribe.module.Module(["test_data", "greet"]), function_name='greet', output_root_dir=pathlib.Path("out/root"), ask_for_test_name=False, ask_for_description=True)
    assert isinstance(result, test_scribe.input_params.InputParams)
    ...
    assert isinstance(result.module, test_scribe.module.Module)
...
def test_create_output_dir_for_module():
    ...
    result = create_output_dir_for_module(output_root_dir=output_root_dir, module=test_scribe.module.Module(["foo", "bar", "m"]))
...
def test_get_scribe_file_path():
    m_create_output_dir_for_module: test_scribe.input_params.create_output_dir_for_module = create_autospec(spec=test_scribe.input_params.create_output_dir_for_module)
    m_create_output_dir_for_module.return_value = pathlib.Path("test_root/a")
    with patch('test_scribe.input_params.create_output_dir_for_module', m_create_output_dir_for_module):
        result = get_scribe_file_path(output_root_dir=pathlib.Path("test_root"), module=test_scribe.module.Module(["a", "b"]))
    assert isinstance(result, pathlib.PosixPath)
    assert repr(result) == "PosixPath('test_root/a/b.tscribe')"
    m_create_output_dir_for_module_mock_calls = get_normalized_mock_calls(m_create_output_dir_for_module, test_scribe.input_params.create_output_dir_for_module)
    ...
    assert isinstance(m_create_output_dir_for_module_mock_calls[0].kwargs['module'], test_scribe.module.Module)
    assert m_create_output_dir_for_module_mock_calls[0].kwargs['module'].names == ('a', 'b')


def test_get_target_class_name_no_class():
    input_params: test_scribe.input_params.InputParams = create_autospec(spec=test_scribe.input_params.InputParams)
