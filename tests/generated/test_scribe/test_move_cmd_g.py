...
def test_do_move_no_test_to_move():
    result = do_move(module=test_scribe.module.Module(["foo", "bar", "m"]), output_root_path=pathlib.Path("a/b"), tests_to_move=[])
...
def test_module_contain_same_symbol_symbol_not_exist():
    result = module_contain_same_symbol(module_str='test_scribe.value_input', name='foo', is_class=False, target_module_str='test_scribe.util')
    assert result is False
...
def test_module_contain_same_symbol_import_same_symbol():
    ...
    """
    result = module_contain_same_symbol(module_str='test_scribe.value_input', name='remove_trailing_numbers', is_class=False, target_module_str='test_scribe.util')
    assert result is False
...
def test_search_tests_to_remove():
    ...
    m_test_model_3: test_scribe.model_type.TestModel = create_autospec(spec=test_scribe.model_type.TestModel)
