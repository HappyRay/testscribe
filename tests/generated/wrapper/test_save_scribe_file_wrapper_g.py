...
def test_save_obj_to_yaml_set():
    result = save_obj_to_yaml(obj=test_scribe.model_type.SetModel([1, 2]))
    assert result == """\
!test_scribe/set
- 1
- 2
...
def test_save_obj_to_yaml_module_model():
    result = save_obj_to_yaml(obj=test_scribe.model_type.ModuleModel("m"))
    assert result == "!test_scribe/module 'm'\n"
...
def test_save_obj_to_yaml_expression_model():
    result = save_obj_to_yaml(obj=test_scribe.model_type.ExpressionModel("a"))
    assert result == "!test_scribe/expression 'a'\n"


def test_save_obj_to_yaml_mock_name_model():
    result = save_obj_to_yaml(obj=test_scribe.model_type.MockNameModel("a"))
    assert result == "!test_scribe/mock 'a'\n"
